...
import com.zk.framework.util.UIUtils;
...
public class BottomBarItem extends LinearLayout {
    ...
    // 是否开启触摸背景，默认关闭
    private boolean openTouchBg = false;
    //触摸时的背景
    ...
    private void checkValues() {
        ...
        if (unreadTextBg == null) {
            unreadTextBg = getResources().getDrawable(R.drawable.bg_bottom_unread);
        }

        if (msgTextBg == null) {
            msgTextBg = getResources().getDrawable(R.drawable.bg_bottom_bar_msg);
        }

        if (notifyPointBg == null) {
            notifyPointBg = getResources().getDrawable(R.drawable.bg_bottom_bar_notify_point);
        }
    }
