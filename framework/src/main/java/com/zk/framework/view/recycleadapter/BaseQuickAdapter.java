 ...
 */
package com.zk.framework.view.recycleadapter;
...
import com.zk.framework.view.recycleadapter.animation.AlphaInAnimation;
import com.zk.framework.view.recycleadapter.animation.BaseAnimation;
import com.zk.framework.view.recycleadapter.animation.ScaleInAnimation;
import com.zk.framework.view.recycleadapter.animation.SlideInBottomAnimation;
import com.zk.framework.view.recycleadapter.animation.SlideInLeftAnimation;
import com.zk.framework.view.recycleadapter.animation.SlideInRightAnimation;
import com.zk.framework.view.recycleadapter.entity.IExpandable;
import com.zk.framework.view.recycleadapter.loadmore.LoadMoreView;
import com.zk.framework.view.recycleadapter.loadmore.SimpleLoadMoreView;
import com.zk.framework.view.recycleadapter.util.MultiTypeDelegate;
...
public abstract class BaseQuickAdapter<T, K extends BaseViewHolder> extends RecyclerView.Adapter<K> {
     ...
     */
    public BaseQuickAdapter(@LayoutRes int layoutResId, @Nullable List<T> data, @Nullable Context mContext) {
    ...
    public BaseQuickAdapter(@Nullable List<T> data, @Nullable Context mContext) {
        this(0, data, mContext);
    }

    public BaseQuickAdapter(@LayoutRes int layoutResId, @Nullable Context mContext) {
        this(layoutResId, null, mContext);
    }
    ...
    public void setNewData(@Nullable List<T> data) {
        ...
        if (dataSetCallBack != null) {
            dataSetCallBack.setData(getItemCount());
        }
    }
    ...
    public void replaceDataNotNotify(@NonNull Collection<? extends T> data) {
        // 不是同一个引用才清空列表
        if (data != mData) {
            mData.clear();
            mData.addAll(data);
    ...
    public boolean isAutoLoadingMore() {
        return isAutoLoadingMore;
    }

    public void setAutoLoadingMore(boolean autoLoadingMore) {
        isAutoLoadingMore = autoLoadingMore;
    }
    ...
    private void bindViewClickListener(final BaseViewHolder baseViewHolder) {
        ...
        //这里修改点击事件和长按事件，修复触发长按时候后依然可以触发点击事件的问题
        view.setOnClickListener(v -> setOnItemClick(v, baseViewHolder.getLayoutPosition() - getHeaderLayoutCount()));
        view.setOnLongClickListener(v -> setOnItemLongClick(v, baseViewHolder.getLayoutPosition() - getHeaderLayoutCount()));
    }
    ...
    public boolean setOnItemLongClick(View v, int position) {
        return mOnItemLongClickListener != null && mOnItemLongClickListener.onItemLongClick(BaseQuickAdapter.this, v, position);
    }
     ...
     * @param payloads-
     */
    public void convert(K helper, T item, List<Object> payloads) {
    ...
    protected boolean hasSubItems(IExpandable item) {
        if (item == null) {
            return false;
        }
    ...
    protected IExpandable getExpandableItem(int position) {
        T item = getItem(position);
        if (isExpandable(item)) {
